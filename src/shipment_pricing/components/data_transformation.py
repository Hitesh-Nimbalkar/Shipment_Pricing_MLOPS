...
class Feature_Engineering(BaseEstimator, TransformerMixin):
    ...
    def replace_nan_with_median(self,X:pd.DataFrame, column_label):
        """
        ...
        """
        median_value = X[column_label].median()
        X[column_label] = X[column_label].fillna(median_value)
        return X
    
    def sub_calssification_modify(self,X):
        X["Sub_Classification"] = np.where(X['Sub_Classification'].str.contains("HIV", case=False), "HIV", X["Sub_Classification"])
        return X
    
    def update_dosage_form(self,X):
    ...
    def replace_low_frequency_categories(self,X, column_name, threshold_percentage=1, replace_value='Others'):
        """
        ...
        category_percentages = X[column_name].value_counts(normalize=True) * 100
        ...
        X[column_name] = np.where(X[column_name].isin(low_frequency_categories), replace_value, X[column_name])

        return X

    ############    Removing Outiers ##############

    def identify_outliers(self,X, numerical_columns, quantile_percentage=0.25, threshold=1.5):
        outliers = pd.DataFrame()

        for column in numerical_columns:
            q1 = np.percentile(X[column], 100 * quantile_percentage / 2)
            q3 = np.percentile(X[column], 100 - (100 * quantile_percentage / 2))
            iqr = q3 - q1
            ...
            outliers_removed = X[column][column_outliers]
            outliers_removed_count = outliers_removed.count()
            logging.info(f"Count of outliers removed from '{column}': {outliers_removed_count}")

        return outliers

    def remove_outliers(self,X, outliers):
        filtered_X = X[~outliers.any(axis=1)]
        # Use logging.info instead of print
        logging.info(f"Shape before outlier removal: {X.shape}")
        logging.info(f"Shape after outlier removal: {filtered_X.shape}")
        return filtered_X

    def get_filtered_dataframe(self,filtered_X):
        if filtered_X is not None:
            return filtered_X
        else:
            logging.info("Filtered DataFrame is not available. Please run remove_outliers() first.")
            return None

    def outlier_removal(self,X, numerical_columns, quantile_percentage=0.25, threshold=1.5):
        outliers = self.identify_outliers(X, numerical_columns, quantile_percentage, threshold)
        filtered_X = self.remove_outliers(X, outliers)
        return filtered_X
    ...
    def run_data_modification(self,data):
        ...
        X=self.sub_calssification_modify(X)
        
        X=self.update_dosage_form(X)
        
        # Replace low frequency with others 
        columns =['Brand','Vendor','Country']
        for column in columns:
            X=self.replace_low_frequency_categories(X=X,column_name=column)
